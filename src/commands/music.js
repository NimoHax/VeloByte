...
const COOKIE_PATH = "/tmp/velobyte-youtube-cookies.txt";

function getCookiePath() {
  const cookies = process.env.YOUTUBE_COOKIES_B64?.trim();
...
async function resolveTrack(query) {
  ...
  const searchQuery = isYoutubeUrl
    ? cleanQuery
    : `ytsearch1:${cleanQuery}`;

  const info =
    await runYtDlpJson(searchQuery);
